<script lang="ts">
  ...
  import { resolveApiKey } from "../lib/providers/claude";
  import {
    clearAiAction,
  ...
  import ApiKeySettings from "../components/ApiKeySettings.svelte";
  ...
  function handleApiKeySave(key: string) {
    apiKey = key;
    addMessage("system", "API key saved. Ready to chat.");
  }
  ...
  onMount(() => {
    ...
    if (apiKey) {
      addMessage(
        "system",
        "AI Claude ready. Ask Claude about your After Effects project."
      );
    }
...
<div class="app">
  <header class="header">
    <span class="header__title">AI Claude</span>
    <div class="header__controls">
      ...
      <select class="model-select" bind:value={model} disabled={!apiKey}>
